...
async def entrypoint(ctx: JobContext):
    ...
    logger.info(f"Using session ID from room name: {session_id}")

    # Create aiohttp session for HTTP requests to backend
    http_session = aiohttp.ClientSession()
    backend_url = f'http://localhost:8000/api/sessions/{session_id}/transcription'
    logger.info(f"Will send transcriptions to: {backend_url}")
    ...
    try:
                            ...
                            if event.alternatives[0].text:
                                try:
                                    async with http_session.post(
                                        backend_url,
                                        json={"text": event.alternatives[0].text}
                                    ) as response:
                                        if response.status == 200:
                                            logger.info(f"Sent transcription to backend successfully")
                                        else:
        ...
        # Wait indefinitely - the agent will stay alive until the room closes
        # This prevents the entrypoint from exiting and closing the HTTP session
        await asyncio.Event().wait()

    finally:
        # Close HTTP session when agent is done
